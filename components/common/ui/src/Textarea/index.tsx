...
function ErrorMessage({ message }: { message: TextAreaProps['errorMessage'] }) {
  return (
    <div className="mr-auto min-h-4 flex-auto break-words pr-2 text-xs color-primary-500">
      {message}
    </div>
  )
}
...
function Counter({ length, isError, setValid }: CounterProps) {
  ...
  const textColor =
    !valid || isError ? 'color-primary-500' : 'color-neutral-500'
  ...
  return hasRange ? (
    <span className={`ml-auto min-h-4 text-xs ${textColor}`}>
      {' '}
      {`${current}/${rangeText}`}{' '}
    </span>
  ) : null
...
export function TextArea({
  ...
  const outlineClassNames = outlined
    ? isError
      ? ERROR_OUTLINE
      : DEFAULT_OUTLINE
    : HIDDEN_OUTLINE
  const statusClassNames = disabled
    ? DISABLED_CLASSNAMES
    : [DEFAULT_CLASSNAMES, ACTIVE_THEME_DICT[theme]].join(' ')
  const classNames = [
    outlineClassNames,
    statusClassNames,
    CONFIG_CLASSNAMES,
